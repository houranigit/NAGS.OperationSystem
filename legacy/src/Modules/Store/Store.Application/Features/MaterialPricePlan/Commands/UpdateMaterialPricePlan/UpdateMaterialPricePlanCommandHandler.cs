...
public sealed class UpdateMaterialPricePlanCommandHandler(
    IMaterialPricePlanRepository plans,
    ICurrencyReader currencies)
    : ICommandHandler<UpdateMaterialPricePlanCommand>
{
    public async Task<Result> Handle(UpdateMaterialPricePlanCommand request, CancellationToken cancellationToken)
    {
        var id = MaterialPricePlanId.From(request.Id);
        var entity = await plans.GetByIdAsync(id, cancellationToken);
        if (entity is null) return Error.NotFound("Material price plan was not found.");
