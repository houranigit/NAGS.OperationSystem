...
public sealed class SuspendContractCommandHandler(
    IContractRepository contracts,
    ICurrentUserService currentUser,
    TimeProvider time)
    : ICommandHandler<SuspendContractCommand>
{
    public async Task<Result> Handle(SuspendContractCommand request, CancellationToken cancellationToken)
    {
        var userId = currentUser.UserId;
        if (userId is null)
            return Error.Unauthorized("Authenticated user is required to suspend a contract.");
        ...
        var result = contract.Suspend(request.Reason, userId.Value, time.GetUtcNow());
