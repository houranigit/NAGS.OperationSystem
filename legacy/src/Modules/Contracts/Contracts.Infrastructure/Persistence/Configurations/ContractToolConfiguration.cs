...
public sealed class ContractToolConfiguration : IEntityTypeConfiguration<ContractTool>
{
    public void Configure(EntityTypeBuilder<ContractTool> builder)
    {
        builder.ToTable("ContractTools", "contracts");
        builder.HasKey(x => x.Id);

        builder.Property(x => x.Id)
            .HasConversion(v => v.Value, v => ContractToolId.From(v))
            .ValueGeneratedNever();
        ...
        {
            t.Property(p => p.ToolId).HasColumnName("Tool_ToolId").IsRequired();
            t.Property(p => p.Name).HasColumnName("Tool_Name").HasMaxLength(100).IsRequired();
        });
        ...
        {
            b.ToTable("ContractToolBrackets", "contracts");
            b.WithOwner().HasForeignKey("ContractToolId");
