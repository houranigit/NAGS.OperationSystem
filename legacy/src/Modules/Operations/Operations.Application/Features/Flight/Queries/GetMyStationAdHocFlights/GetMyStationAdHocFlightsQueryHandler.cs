...
/// <summary>
/// Station-scoped Ad Hoc flights — same projection as <see cref="GetMyStationAogFlights.GetMyStationAogFlightsQueryHandler"/>
/// but membership is <c>OperationTypeId == AdHoc</c> instead of carrying an AOG service.
/// </summary>
public sealed class GetMyStationAdHocFlightsQueryHandler(
    IOperationsDbContext db,
    IEmployeeReader employeeReader,
    IWorkOrderRepository workOrders)
    : IQueryHandler<GetMyStationAdHocFlightsQuery, IReadOnlyList<MobileAogFlightSummaryDto>>
{
    private sealed record AdHocFlightPageRow(
        Guid Id,
        string FlightNumber,
        string CustomerName,
    ...
    public async Task<Result<IReadOnlyList<MobileAogFlightSummaryDto>>> Handle(
        GetMyStationAdHocFlightsQuery request,
        CancellationToken cancellationToken)
    {
        ...
        var pageRows = await db.Flights
            ...
            .Select(f => new AdHocFlightPageRow(
                f.Id.Value,
