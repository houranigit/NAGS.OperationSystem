...
fun WorkOrderServicePicker(
    ...
    InlineSearchableDropdownField(
        ...
        secondaryLine = { svc -> if (svc.isAog) "AOG" else null },
        onSelect = onSelected,
...
fun WorkOrderEmployeePicker(
    ...
    val selected = options.firstOrNull { it.employeeId == selectedId }
    InlineSearchableDropdownField(
        label = "Performed by",
        ...
        secondaryLine = { emp -> emp.stationCode.takeIf { s -> s.isNotBlank() } },
        onSelect = onSelected,
        readOnly = readOnly || options.isEmpty(),
        modifier = modifier,
        isError = isError,
