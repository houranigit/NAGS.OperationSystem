...
/**
 * AOG tab body — sits under [com.nags.operations.ui.components.AppHeader]
 * inside [com.nags.operations.ui.home.MainShellScreen].
 */
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun AogFlightsTab(
    viewModel: AogFlightsViewModel,
    sheetCallbacks: FlightSheetCallbacks = FlightSheetCallbacks(),
) {
    var sheetFlight by remember { mutableStateOf<MobileFlightSummaryDto?>(null) }
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        OutlinedTextField(
            ...
            placeholder = { Text("Search for a Per Landing flight..") },
        ...
        PullToRefreshBox(
            ...
            when {
                ...
                state.error != null && state.items.isEmpty() -> Box(
                    ...
                    ErrorState(
                        title = "Couldn't load AOG flights",
                        message = state.error!!,
                ...
                state.items.isEmpty() -> Box(
                    ...
                    EmptyState(
                        ...
                        title = if (noFiltersActive) "No AOG flights right now" else "No matching AOG flights",
                        message = when {
                            noFiltersActive ->
                                "No AOG flights at your station in the next 12 hours. Pull down or tap refresh to check again."
    ...
    sheetFlight?.let { f ->
        FlightDetailsActionsSheet(
            ...
            callbacks = FlightSheetCallbacks(
                ...
                onCancelFlight = { id, canceledAtIso ->
                    sheetFlight = null
                    viewModel.cancelFlight(id, canceledAtIso)
                },
            ),
