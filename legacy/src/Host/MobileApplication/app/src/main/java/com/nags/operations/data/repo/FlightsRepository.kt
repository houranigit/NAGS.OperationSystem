...
/**
 * Reactive reads for both flight lists. The sync coordinator owns the writes
 * so this class is genuinely read-only — there's no `refresh()` here on
 * purpose, the UI triggers refreshes through the sync coordinator's public
 * surface so every refresh shows up in the Sync Center.
 */
class FlightsRepository(private val db: AppDatabase) {
    /** Non-AOG flights the signed-in user is rostered on, sorted by STA ascending. */
    ...
    /** AOG flights at the signed-in user's home station, sorted by STA ascending. */
    fun aogFlightsFlow(): Flow<List<AogFlightEntity>> = db.aogFlightDao().observeAll()
     ...
     * AOG, or Ad Hoc). Used when navigating into create-work-order from a list row.
     */
    suspend fun findWorkOrderFlight(id: String): WorkOrderFlightRow? =
        db.flightDao().getById(id)?.toWorkOrderFlightRow()
            ?: db.aogFlightDao().getById(id)?.toWorkOrderFlightRow()
