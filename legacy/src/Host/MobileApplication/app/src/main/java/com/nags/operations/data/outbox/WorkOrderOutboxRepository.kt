...
class WorkOrderOutboxRepository(
    ...
    /**
     * Outbox state overlay for the My / AOG / AdHoc lists. The key is the
    ...
    suspend fun enqueueCancel(
        ...
        EnqueueRequest(
            ...
            payload = OutboxPayload(
                ...
                workOrder = null,
                cancelFlight = OutboxPayload.CancelFlightInput(canceledAtIso = canceledAtIso),
            ),
    ...
    /**
     * Convenience enqueue for "update the cancellation time of an existing under-review
     * cancel work order". Reuses the [OutboxPayload.Kind.UpdateExisting] path (PUT
     * `.../work-orders/{workOrderId}`) with empty line collections — a cancel work order
     * has no service / task lines — and [OutboxPayload.WorkOrderInput.isCanceled] set so
     * the server keeps it a cancellation with the new [canceledAtIso] time.
     */
    ...
    private fun WorkOrderOutboxEntity.toPendingAdHoc(json: Json): PendingAdHocFlight? {
        ...
        val workOrder = payload.workOrder ?: return null
