...
- IATA code is required, is normalized to uppercase on the backend, and must be globally unique across Customers.
- A Customer IATA code is exactly two uppercase letters or digits.
...
- Database unique constraints enforce Station IATA, non-null Station ICAO, Customer IATA, and non-null Customer ICAO uniqueness in addition to application validation.
...
Identity must validate Roles against the combined permission catalogs registered by all modules. The current Identity-only `IdentityPermissions.IsKnown` check is insufficient once MasterData permissions exist and must be replaced by a modular catalog contract.
...
- The current Identity global unique index on `users.Email` cannot support released-email reuse as written and must be redesigned while preserving the old User and its historical email for audit.
...
12. Station IATA/ICAO and Customer IATA/ICAO codes are backend-normalized to uppercase and uniquely constrained; Station/Customer ICAO is optional.
...
15. The first usable release includes complete Country, ManpowerType, License, Station, Customer, CustomerContact, and StaffMember management across backend and Blazor UI.
...
Country, ManpowerType, and License are prerequisite catalog slices. Station and Customer follow Country. StaffMember follows Station, ManpowerType, and License. Portal-access integration may begin after the Identity changes (`UserType`, role compatibility, external reference, email reverification, released-email semantics, and integration messaging) are designed as implementation slices.
