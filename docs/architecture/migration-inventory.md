...
| Service (incl. seeded Aircraft Per Landing, On Call) | done | `Core.Domain/Aggregates/Service/*`, `Features/Service/**` | `MasterData.*/Features/Services` | Full stack: name/description catalog with protected system rows; seeded Aircraft Per Landing and On Call services with stable ids; rowversion/ETag; `/api/v1/masterdata/services` + permissions; typed client + list/detail/dialog UI + nav; domain + integration tests |
...
| MasterData seeding + well-known ids | done | `Core.Infrastructure/Seeding/CoreDataSeeder.cs`, `Core.Contracts/Seeding/CoreSeedIds.cs` | `MasterData.Infrastructure/Seeding`, `MasterData.Contracts/Seeding` | Current MVP baseline: idempotent ISO country seed plus protected OperationType/Service seed rows with stable ids. Additional future catalog seeds can extend the same pattern |
...
See `docs/modules/operations-foundation.md` (domain design, incl. the 2026-07-04 corrections) and `docs/modules/operations-module-plan.md` (build plan). Backend + portal implemented for the full schedule-first and work-order-first cycles: no `PendingReview` flight status (submit keeps the flight InProgress), approved work-order values captured onto the flight (billing loads flight + approved WO), owner-scoped work orders, backend-enforced visibility, per-flight timeline, and a calendar page.

| Feature | Status | Old reference files | New location | Notes |
|---|---|---|---|---|
| Flight scheduling (schedule, update, change-number) | done | `Operations.Domain/Aggregates/Flight/Flight.cs`, `Operations.Application/Features/Flight/Commands/{CreateFlight,BatchCreateFlights,UpdateFlight}/*` | `Operations.Domain/Flights/Flight.cs`, `Operations.Application/Features/Flights/FlightCommands.cs` | Snapshots via MasterData read seam; original+current flight number; planned services mandatory (per-landing mixing rule); batch create still pending |
| Per-Landing flights (claim, auto-WO job, station-wide visibility) | done | `ClaimAogFlight/*`, `AutoAogWorkOrderJob.cs` | `Features/Flights/FlightLifecycleCommands.cs` (ClaimPerLandingFlight), `Operations.Infrastructure/BackgroundJobs/AutoWorkOrderJob.cs` | Aircraft Per Landing = no-service designation; not performable as actual service; station-wide visibility enforced server-side; auto-WO at STD+60m (appsettings) |
| Ad-hoc flights + work order from scratch | done | `CreateAdHocWorkOrderFromScratch/*` | `Features/Flights/FlightLifecycleCommands.cs` (CreateAdHocFlightWithWorkOrder), Blazor `AdHocWorkOrderDialog.razor` | Collects flight planning + work-order actual fields (incl. cancellation variant); planned services required unless cancellation; duplicate detection warn/link/flag; flight appears on calendar + list |
...
| StaffMember flight assignment + assigned-only visibility | done | `InviteEmployeeToFlight/*`, `FlightAssignedEmployee.cs` | `Features/Flights/FlightCommands.cs` (AssignEmployees), `Application/Authorization/OperationsScope.cs` (EnsureFlightAccess) | Non-per-landing flights visible/authorable only by assigned staff (server-enforced on reads and writes); assigned staff can invite others; timeline entry per assignment; Notifications integration event pending |
| Work order authoring (owner-scoped; service lines, tasks, resources, attachments, RTR) | done | `WorkOrder.cs`, `WorkOrderTask*.cs`, `{CreateWorkOrderForFlight,UpdateWorkOrder,RecordReturnToRampLines}/*` | `Domain/WorkOrders/*`, `Features/WorkOrders/WorkOrderCommands.cs` | Owner (`OwnerStaffMemberId` + snapshot) captured at open; only owner/admin edits or submits; one active WO per staff per flight, multiple staff per flight; editable actual flight number + actual aircraft type; opening a draft does NOT change flight status; actual services optional |
| Cancellation as work order | done | `CancelFlight/*` | `Features/Flights/FlightLifecycleCommands.cs` (CancelFlight) | Cancellation = work order (numbered, billable); requires only cancellation date/time (+optional reason); flight stays InProgress until approval |
| Work order review (submit, approve→capture+billing, reject, return→clear) | done | `{ApproveWorkOrder,RejectWorkOrder,RevokeWorkOrder}/*` | `Features/WorkOrders/{WorkOrderCommands,WorkOrderReviewCommands}.cs` | Submit keeps flight InProgress (no PendingReview); approve assigns number, captures `ApprovedWorkOrderSnapshot` + reference onto the flight, publishes enriched `FlightSentToBilling`; completion approval requires actual aircraft type/flight number/ATA/ATD; return wipes number + clears flight snapshot + reverts to InProgress; re-approval draws a new number |
| Per-station work order numbering | done | `StationWorkOrderCounter.cs` | `Domain/Sequences/StationWorkOrderSequence.cs`, `Infrastructure/Persistence/WorkOrderNumberAllocator.cs` | Serializable allocation; `{IATA}-{nnnn}`; numbers never reused after return (gaps visible in timeline) |
| Flight queries (list, by id, calendar, timeline, billing view) | done | `{GetFlightById,GetPaginatedFlights,GetFlightLightsInPeriod}/*` | `Features/Flights/FlightQueries.cs` | Station- and assignment-scoped; detail includes captured approved values + WO owners; `GET /flights/{id}/timeline`; billing view = flight snapshot + referenced approved WO detail |
...
| Blazor portal: dashboard, flights list, calendar, flight detail, WO dialogs, review queue | done | (Host.Web pages incl. `Pages/Scheduler`) | `OperationsSystem.Blazor.Client/Features/Operations/**`, `Api/OperationsApiClient.cs` | Full cycle testable in portal: schedule/ad-hoc create → author (ownership-aware) → submit → review/approve → captured values + timeline on detail → return/re-approve; calendar page (`/operations/calendar`, RadzenScheduler) shows scheduled + ad-hoc flights |
| Customer signature capture (work order) | done | `WorkOrder.cs` (CustomerSignature) | `Domain/WorkOrders/WorkOrder.cs` (CustomerSignatureReference) | Optional; stores storage reference, not bytes; captured onto flight snapshot on approval |
...
| Ordered migrations apply | done | `Host.Web/Extensions/DatabaseMigrationExtensions.cs` | `OperationsSystem.Api/Program.cs` | Startup applies Audit → Identity → MasterData in dev; production uses reviewed scripts. First-release migration squash deferred to environment cutover. |
