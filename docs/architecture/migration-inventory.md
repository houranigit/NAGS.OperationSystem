...
| React portal: auth + Users/Roles/Sessions screens | done | (Blazor `Components/Pages/Settings/System/**`) | `src/Host/OperationsSystem.Web/**` | Vite/React/TS/Tailwind/TanStack Query/RHF/Zod/Router; shadcn/ui (radix-nova) with RTL; i18n scaffolding (react-i18next, en + ar stub, `Accept-Language`); login, activation, users (list/detail/invite/edit/role/lifecycle), roles (CRUD + permissions), per-user + self-service session management, account (profile/change-password/sessions); permission-gated routing; in-memory token + refresh cookie |
