...
- Mobile reuses the same domain and business rules — normal flights copy planned services into work-order lines while Per-Landing work orders start empty, one active work order per user per flight, clear-and-rebuild service lines, task reconciliation by stable id. A Per-Landing flight becomes On Call after any real performed service line is persisted on a non-merged work order. Mobile has no business logic of its own.
