...
- Work-order service lines and tasks use the same attachment kinds, validation policy, storage abstraction, upload/download/delete behavior, per-owner limit, and approval lock. Each metadata row belongs to exactly one service line or task.
- Service lines and tasks reconcile by stable child id during work-order updates so retained children keep their attachment metadata. Removing a child or deleting a work order also cleans up its stored files; a failed multi-file operation cleans up files already written by that operation.
- The legacy `Operations.Domain/Entities/{WorkOrderTask.cs,WorkOrderTaskAttachment.cs}` and `Host/MobileApplication/app/src/main/java/com/nags/operations/data/outbox/{OutboxPayload.kt,WorkOrderOutboxRepository.kt}` supported task attachments only. A service-line attachment collection did not exist there; it is intentionally new in v1.0.0.
...
- For aggregates with large child graphs, clearing and rebuilding is acceptable only when no durable child identity or owned state must survive. Contract pricing lines may use that approach where their business rules allow it.
- Work Order service lines and tasks reconcile by stable id. Retained rows are updated in place, omitted rows are removed, new rows receive server ids, and duplicate or foreign incoming ids reject the update before the child collection is mutated. This preserves service/task attachment ownership across edits.
...
- Well-known seed entities (currencies, default country, the Ad Hoc operation type, Aircraft Per Landing service, etc.) use fixed, stable GUIDs that are identical across all environments and never regenerated. Domain rules may depend on these ids. On Call is a derived Operations state and deliberately has no seed row or GUID.
...
- Mobile reuses the same domain and business rules — normal flights copy planned services into work-order lines while Per-Landing work orders start empty, one active work order per user per flight, and service/task reconciliation by stable id. Its durable outbox stores attachment files outside Room payload JSON and maps them back to the owning service line or task when sending. A Per-Landing flight becomes On Call after any real performed service line is persisted on a non-merged work order. Mobile has no business logic of its own.
...
| 2026-06-21 | Users have one business/data-scope type plus one compatible permission-bearing Role; linked types are immutable while direct types follow the governed transition recorded below | Separates MasterData-owned identity scope from configurable authorization capabilities without preventing controlled direct-account promotion/demotion |
...
| 2026-07-24 | Allow atomic access transitions between the two direct account types (`SystemAdministrator` and `ViewerOnly`) while keeping linked account types immutable | A selected Role remains authoritative; account-type changes require separate authority, preserve MasterData ownership boundaries, protect the last break-glass administrator, reject stale writes, and invalidate old tokens/sessions immediately |
...
| 2026-07-11 | Mobile idempotency keys are bound to the authenticated owner, mutation kind, target, and a request fingerprint; offline updates carry their cached base `RowVersion` | A retried response replays safely, a reused key cannot acknowledge different work, and stale field work cannot silently replace a newer portal edit |
...
| 2026-07-18 | On Call is derived for Per-Landing flights from non-merged work orders with at least one performed service line; the seeded On Call catalog service is retired | Staff record the actual service performed, while list/detail/calendar/filter/extraction behavior uses one consistent state rule |
| 2026-07-23 | Work-order service lines and tasks reconcile by stable id and both may own attachment metadata backed by external file storage | Service attachments must mirror task behavior without losing existing files during an otherwise ordinary work-order edit; legacy supported task attachments only |
