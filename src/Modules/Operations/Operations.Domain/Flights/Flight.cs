...
/// its work order(s) by id. Completed/Canceled are terminal, post-approval, billable outcomes.
/// </summary>
public sealed class Flight : AggregateRoot<Guid>
{
    ...
    public void OnWorkOrderOpened(DateTimeOffset now)
    {
    ...
    public Result SettleCompleted(Guid workOrderId, DateTimeOffset now)
    {
        Status = FlightStatus.Completed;
        UpdatedAtUtc = now;
        RaiseDomainEvent(new FlightCompleted(Id, workOrderId));
        return Result.Success();
    }

    public Result SettleCanceled(Guid workOrderId, DateTimeOffset now)
    {
        Status = FlightStatus.Canceled;
        UpdatedAtUtc = now;
        RaiseDomainEvent(new FlightCanceled(Id, workOrderId));
        return Result.Success();
    }
