...
interface PerLandingFlightDao {
    @Query(
        """
        SELECT * FROM flights_per_landing
    ...
    @Query(
        """
        SELECT COUNT(*) FROM flights_per_landing
        WHERE isPerLanding = 1
          AND isAdHoc = 0
          AND status IN ('Scheduled', 'InProgress', 'Completed')
        """,
...
interface AdHocFlightDao {
    ...
    @Query(
        """
        SELECT COUNT(*) FROM flights_ad_hoc
        WHERE isAdHoc = 1
          AND status IN ('Scheduled', 'InProgress', 'Completed')
        """,
    )
