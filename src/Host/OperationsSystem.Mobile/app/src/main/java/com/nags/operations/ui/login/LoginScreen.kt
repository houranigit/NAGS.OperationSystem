...
fun LoginScreen(
    ...
    Column(
        ...
        Column(
            ...
            // Two-step MFA: the server answered the first step with a challenge — collect the
            // authenticator code and finish sign-in with it.
            ...
            Button(
                onClick = {
                    if (state.mfaToken != null) viewModel.confirmMfa(onLoggedIn)
                    else viewModel.login(onLoggedIn)
                },
                enabled = !state.isLoading && state.email.isNotBlank() && state.password.isNotBlank() &&
                    (state.mfaToken == null || state.mfaCode.isNotBlank()),
                modifier = Modifier
                    .fillMaxWidth()
                    .height(54.dp),
                shape = RoundedCornerShape(14.dp),
